...
mod1<-glm.nb(death_text~network2+offset(log(wc)),data=results)
mod2<-glm.nb(death_text~network2*week2+offset(log(wc)),data=results)

#export table
stargazer(mod1,mod2,intercept.bottom = F,title="Table 3: Fox News is Significantly Less Likely to Use Words from LIWC's 'Death' Category When Discussing Covid-19",dep.var.labels=c("'Death' Mentions"),covariate.labels=c('Constant','CNN','MSNBC','Week','CNN X Week','MSNBC X Week'),type='html',out='output/table3.html')
